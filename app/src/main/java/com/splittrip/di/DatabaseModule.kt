...
object DatabaseModule {
    ...
    fun provideDatabase(@ApplicationContext context: Context): SplitTripDatabase {
        return Room.databaseBuilder(
            context,
            SplitTripDatabase::class.java,
            "splittrip_database"
        ).build()
    }

    @Provides
    fun provideTripDao(db: SplitTripDatabase): TripDao = db.tripDao()

    @Provides
    fun provideMemberDao(db: SplitTripDatabase): MemberDao = db.memberDao()

    @Provides
    fun provideExpenseDao(db: SplitTripDatabase): ExpenseDao = db.expenseDao()
}
